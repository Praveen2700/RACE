...
def login():
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            # Retrieve login attempt info from the session
            if 'login_attempts' not in session:
                session['login_attempts'] = 0
                session['lockout_time'] = None

            if session['lockout_time']:
                cooldown_remaining = time() - session['lockout_time']
                if cooldown_remaining < COOLDOWN_PERIOD:
                    # User is in cooldown
                    flash(f'Please wait {int(COOLDOWN_PERIOD - cooldown_remaining)} seconds before trying again.', 'error')
                    return render_template('login.html', username=username1)

            # Perform PostgreSQL query to check if the username exists
            cur.execute("SELECT * FROM login WHERE username = %s", (caesar_cipher(username1, shift),))
            user = cur.fetchone()
            ...
            if user:
                if user[-2] == password:
                    ...
                    # Increment login attempts and check if it's exceeded the limit
                    session['login_attempts'] += 1
                    if session['login_attempts'] >= LOGIN_ATTEMPT_LIMIT:
                        # Set the lockout time
                        session['lockout_time'] = time()
                        flash('Too many failed attempts. Please wait 5 minutes before trying again.', 'error')
                    else:
...
def send_email(schedule_details):
    # Email content
    subject = 'Schedule Details'
    body = f'''
    <h2>Schedule Details</h2>
    <p><b>Course Name:</b> {schedule_details['CourseName']}</p>
    <p><b>Mentor Name:</b> {schedule_details['MentorName']}</p>
    <p><b>Program Name:</b> {schedule_details['ProgramName']}</p>
    <p><b>Batch Name:</b> {schedule_details['BatchName']}</p>
    <p><b>Schedule Date:</b> {schedule_details['SDate']}</p>
    <p><b>Timing:</b> 10:00 AM - 5:30 PM</p>
    <p><b>Breaks:</b> Two short breaks of 15 minutes each, Lunch break of 45 minutes</p>
    <p><b>MS Teams Link:</b> your_ms_teams_link</p>
    '''
    
     # Create a message instance for sending the email
    msg = Message(subject=subject, recipients=['recipient_email@example.com'], html=body)
